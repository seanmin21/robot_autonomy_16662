import random
from functools import lru_cache
from tictactoe import TicTacToe
from agent import QAgent, save_agent


# ── Minimax ───────────────────────────────────────────────────────────────────

@lru_cache(maxsize=None)
def minimax(state, is_maximizing):
    """
    Cached minimax over board states (tuples).
    Returns +1 if X wins, -1 if O wins, 0 for draw.
    """
    board = list(state)

    # Check terminal conditions
    for p, score in [(1, 1), (-1, -1)]:
        for i in range(3):
            if all(board[i*3 + j] == p for j in range(3)):
                return score
            if all(board[j*3 + i] == p for j in range(3)):
                return score
        if board[0] == board[4] == board[8] == p:
            return score
        if board[2] == board[4] == board[6] == p:
            return score

    available = [i for i, v in enumerate(board) if v == 0]
    if not available:
        return 0  # Draw

    if is_maximizing:
        best = -float("inf")
        for move in available:
            board[move] = 1
            best = max(best, minimax(tuple(board), False))
            board[move] = 0
        return best
    else:
        best = float("inf")
        for move in available:
            board[move] = -1
            best = min(best, minimax(tuple(board), True))
            board[move] = 0
        return best


def minimax_move(state):
    """Return the best move for X (maximizing player) given the board state."""
    board = list(state)
    available = [i for i, v in enumerate(board) if v == 0]
    best_score = -float("inf")
    best_action = None
    for move in available:
        board[move] = 1
        score = minimax(tuple(board), False)
        board[move] = 0
        if score > best_score:
            best_score = score
            best_action = move
    return best_action


# ── Training ──────────────────────────────────────────────────────────────────

def train(episodes=50000):
    print(f"Training for {episodes} episodes against minimax opponent...")
    print("(Agent plays as O, minimax plays as X)\n")
    agent = QAgent(epsilon=0.3, alpha=0.5, gamma=0.9)
    env = TicTacToe()

    for i in range(episodes):
        env.reset()
        done = False

        prev_state = None
        prev_action = None

        # Decay epsilon from 0.3 down to 0.01 over training
        agent.epsilon = max(0.01, 0.3 * (1 - i / episodes))

        while not done:
            state = env.get_state()

            if env.current_player == 1:
                # Minimax plays as X — perfectly
                action = minimax_move(state)
            else:
                # Q-agent plays as O — learning
                action = agent.choose_action(state, env.available_moves())

            reward, done = env.make_move(action)
            next_state = env.get_state()

            if done:
                if reward == 1:
                    # Minimax (X) won — penalise agent's last move
                    if prev_state is not None:
                        agent.learn(prev_state, prev_action, -1.0, next_state, done)
                elif reward == 0.5:
                    # Draw — reward agent's last move
                    if prev_state is not None:
                        agent.learn(prev_state, prev_action, 0.5, next_state, done)
                else:
                    # Agent (O) won — reward the winning move
                    agent.learn(state, action, 1.0, next_state, done)
            else:
                # Mid-game, just finished agent's turn
                if env.current_player == 1 and prev_state is not None:
                    agent.learn(prev_state, prev_action, 0.0, next_state, done)

            # Track previous state/action only for agent's moves
            if env.current_player == 1 and not done:
                prev_state = state
                prev_action = action

        if i % 10000 == 0:
            print(f"  Episode {i:>6} | epsilon: {agent.epsilon:.3f} "
                  f"| Q-table states: {len(agent.q_table)}")

    print(f"\nTraining complete. Q-table has {len(agent.q_table)} states.")
    return agent


# ── Evaluation ────────────────────────────────────────────────────────────────

def evaluate_vs_minimax(agent, n_games=1000):
    """Evaluate agent (as O) against perfect minimax (as X)."""
    print(f"\nEvaluating {n_games} games vs minimax (agent=O, minimax=X)...")
    env = TicTacToe()
    results = {"win": 0, "draw": 0, "loss": 0}

    for _ in range(n_games):
        env.reset()
        done = False
        while not done:
            state = env.get_state()
            if env.current_player == 1:
                action = minimax_move(state)
            else:
                action = agent.choose_action(state, env.available_moves())
            reward, done = env.make_move(action)

        if reward == 1 and env.current_player == -1:
            results["win"] += 1
        elif reward == 0.5:
            results["draw"] += 1
        else:
            results["loss"] += 1

    total = n_games
    print(f"  Win:  {results['win']:>4}  ({100 * results['win'] / total:.1f}%)")
    print(f"  Draw: {results['draw']:>4}  ({100 * results['draw'] / total:.1f}%)")
    print(f"  Loss: {results['loss']:>4}  ({100 * results['loss'] / total:.1f}%)")
    print("\n  Against a perfect minimax, draws are the best achievable outcome.")


def evaluate_vs_random(agent, n_games=1000):
    """Evaluate agent (as X) against a random opponent (as O)."""
    print(f"\nEvaluating {n_games} games vs random opponent (agent=X, random=O)...")
    env = TicTacToe()
    results = {"win": 0, "draw": 0, "loss": 0}

    for _ in range(n_games):
        env.reset()
        done = False
        while not done:
            state = env.get_state()
            if env.current_player == 1:
                action = agent.choose_action(state, env.available_moves())
            else:
                action = random.choice(env.available_moves())
            reward, done = env.make_move(action)

        if reward == 1 and env.current_player == 1:
            results["win"] += 1
        elif reward == 0.5:
            results["draw"] += 1
        else:
            results["loss"] += 1

    total = n_games
    print(f"  Win:  {results['win']:>4}  ({100 * results['win'] / total:.1f}%)")
    print(f"  Draw: {results['draw']:>4}  ({100 * results['draw'] / total:.1f}%)")
    print(f"  Loss: {results['loss']:>4}  ({100 * results['loss'] / total:.1f}%)")


# ── Entry point ───────────────────────────────────────────────────────────────

if __name__ == "__main__":
    trained_agent = train(episodes=50000)
    evaluate_vs_minimax(trained_agent, n_games=1000)
    evaluate_vs_random(trained_agent, n_games=1000)
    save_agent(trained_agent, "qtable.pkl")
