...
def _is_winner_flat(board, p):
    """Check 3-in-a-row win on a flat 16-element list for a 4x4 board."""
    # Rows
    for r in range(4):
        for c in range(2):
            if board[r*4+c] == board[r*4+c+1] == board[r*4+c+2] == p:
                return True
    # Columns
    for c in range(4):
        for r in range(2):
            if board[r*4+c] == board[(r+1)*4+c] == board[(r+2)*4+c] == p:
                return True
    # Diagonal top-left → bottom-right
    for r in range(2):
        for c in range(2):
            if board[r*4+c] == board[(r+1)*4+c+1] == board[(r+2)*4+c+2] == p:
                return True
...
def run_episode(agent, env, agent_player, opponent):
    env.reset()
    done = False
    prev_state = None
    prev_action = None
...
def train(episodes=500000):
    """
    Train the agent on 4x4 Tic-Tac-Toe (3-in-a-row wins).
    Uses self-play and random opponents, alternating sides each episode.
    """
    print(f"Training for {episodes} episodes on 4x4 board (3-in-a-row wins)...")
...
if __name__ == "__main__":
    ...
    save_agent(trained_agent, "qtable4.pkl")
